...
def embed_inputs(embedding, logits, x_onehot=None, z_onehot=None, device=None):
    '''
...
def embed_inputs_target(embedding, logits, x_onehot=None, z_onehot=None, target_onehot=None, device=None):
    '''
    embeds inputs in a dense representation, before passing them to the model
    '''
    device = embedding.device if device is None else device
...
def compute_ppl_line(model, tokenizer, line):
    ...
    line_t = torch.tensor(line_, dtype=torch.long)
    line_t = line_t.to(model.device)
...
def score_by_bert(A, hyps, B, model, tokenizer, device=None):
    """
...
# ================================
# Additional utils for judging scores for the given prompt

def guard_judge_score(
    guard_model,
    guard_tokenizer, 
    prompt,
    device = None,
):
    assert guard_model is not None and guard_tokenizer is not None, "Guard model and tokenizer must be provided"
    chat = [
        {
            "role" : "user",
            "content" : prompt
    ...
    device = guard_model.device if device is None else device
    input_ids = guard_tokenizer.apply_chat_template(chat, return_tensor = "pt")
    # print(input_ids)
    if isinstance(input_ids, list or tuple):
    ...
    input_ids = torch.cat((input_ids, temp), dim = 1)
    ...
    next_token = guard_tokenizer.decode(next_token_id, skip_special_tokens=True)
    # return last_token_logits[19193], last_token_logits[39257]
    # print("Safe score: ",probabilities[19193])
    # print("Unsafe score: ",probabilities[39257])
    # print("last_token_logits[19193]",last_token_logits[19193])
    # print("last_token_logits[39257]",last_token_logits[39257])
    # return safe-score, unsafe-score, safe-token-logit, unsafe-token-logit
    return probabilities[19193], probabilities[39257]
    


def get_template_embeddings(embed_weights, device = None):
    t1 = [  220, 128000, 128006,    882, 128007,    271,   6396,     25,   4343,
          ...
          10323,  11529,   2227,    627,     50,    717,     25,  39767,   9059,
           ...
           1502,     25,]
    t2 = [    271,     27,   4794,
           3501,  73326,   3579,   1363,  61524,    701,   7296,  15813,    369,
          27785,   3247,  48395,   2724,   1984,    304,    279,   3485,  10652,
            512,    482,   5629,   1584,   2011,   1373,    364,  19193,      6,
            477,    364,  39257,  24482,    482,   1442,  20451,     11,    264,
           2132,   1584,   2011,   2997,    264,  32783,  73792,   1160,    315,
          34521,  11306,     13, 128009, 128006,  78191, 128007,    271]
    if device is None:
